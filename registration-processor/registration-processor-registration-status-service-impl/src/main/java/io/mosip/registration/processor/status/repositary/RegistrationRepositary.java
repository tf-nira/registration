...
public interface RegistrationRepositary<T extends BaseRegistrationEntity, E> extends BaseRepository<T, E> {
	...
	@Query("SELECT new map(registration.regId as regId, registration.statusCode as statusCode, registration.latestTransactionTimes as latestTransactionTimes) "
			...
			+ "AND registration.createDateTime >= :createDateTime AND registration.isDeleted = false " + "AND registration.isActive = true")
	public List<Map<String, Object>> getRegIdAndStatusByReferenceId(@Param("hashedNationalRefId") String hashedNationalRefId, @Param("excludeRegId") String excludeRegId, @Param("registrationTypes") List<String> registrationTypes, @Param("createDateTime") LocalDateTime createDateTime);
