...
public interface RegistrationRepositary<T extends BaseRegistrationEntity, E> extends BaseRepository<T, E> {
	...
	public List<RegistrationStatusEntity> getUnProcessedPackets(@Param("status") List<String> status,
			@Param("reprocessCount") Integer reprocessCount, @Param("timeDifference") LocalDateTime timeDifference,
			@Param("statusCodes") List<String> statusCodes, @Param("fetchSize") Integer fetchSize,
	...
	@Query(value = "SELECT * FROM registration r WHERE r.status_code =:statusCode AND r.process IN :includeProcesses AND r.reg_stage_name NOT IN :excludeStageNames AND r.upd_dtimes <:timeDifference order by r.upd_dtimes LIMIT :fetchSize ", nativeQuery = true)
