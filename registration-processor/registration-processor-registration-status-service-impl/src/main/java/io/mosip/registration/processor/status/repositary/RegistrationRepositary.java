...
public interface RegistrationRepositary<T extends BaseRegistrationEntity, E> extends BaseRepository<T, E> {
	...
	@Query("SELECT registration FROM RegistrationStatusEntity registration WHERE registration.regId in :regIds and registration.statusCode !=:statusCode and registration.registrationType not in :registrationTypes")
	public List<RegistrationStatusEntity> getWithoutStatusCodeAndRegistartionType(@Param("regIds") List<String> regIds,
			@Param("statusCode") String statusCode, @Param("registrationTypes") List<String> registrationTypes);
	...
	@Query(value = "SELECT * FROM registration r WHERE r.latest_trn_status_code IN :status AND r.process='MIGRATOR AND r.reg_process_retry_count<=:reprocessCount AND r.latest_trn_dtimes <:timeDifference AND r.status_code NOT IN :statusCodes AND r.reg_stage_name NOT IN :excludeStageNames  order by r.latest_trn_dtimes LIMIT :fetchSize ", nativeQuery = true)
	...
	@Query(value ="SELECT * FROM registration r WHERE r.status_code =:statusCode and r.process='MIGRATOR' order by r.upd_dtimes LIMIT :fetchSize ", nativeQuery = true)
