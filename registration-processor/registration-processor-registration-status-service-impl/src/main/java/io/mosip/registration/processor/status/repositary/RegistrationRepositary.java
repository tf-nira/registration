...
public interface RegistrationRepositary<T extends BaseRegistrationEntity, E> extends BaseRepository<T, E> {
	...
	@Query(value ="SELECT * FROM registration r WHERE r.upd_dtimes < CURRENT_DATE + INTERVAL '1 day' and r.is_anonymous_profile_added IS FALSE order by r.upd_dtimes LIMIT :fetchSize ", nativeQuery = true)
