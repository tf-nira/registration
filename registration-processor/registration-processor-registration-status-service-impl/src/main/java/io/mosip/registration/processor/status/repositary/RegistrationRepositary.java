...
public interface RegistrationRepositary<T extends BaseRegistrationEntity, E> extends BaseRepository<T, E> {
	...
	@Query(value = "SELECT * FROM registration r WHERE r.status_code =:statusCode AND r.registrationStageName NOT IN :excludeStageNames order by r.upd_dtimes LIMIT :fetchSize ", nativeQuery = true)
